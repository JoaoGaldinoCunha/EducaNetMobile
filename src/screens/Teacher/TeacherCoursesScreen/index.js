...
export const TeacherCourseScreen = ({ navigation }) => {
  ...
  const fetchCourses = async () => {
    try {
      const response = await fetch('http://192.168.0.17:8080/course/AllCourses');
  ...
  const handleCreateCourse = async () => {
    ...
    try {
      const response = await fetch('http://192.168.0.17:8080/course/coursesave', {
        method: 'POST',
        headers: {
  ...
  const handleDeleteCourse = async (courseId) => {
    try {
      const response = await fetch(`http://192.168.0.17:8080/course/deleteCourse/${courseId}`, {
        method: 'DELETE',
      });
  ...
  const handleEditCourse = async (courseId, updatedCourseData) => {
    try {
      const response = await fetch(`http://192.168.0.17:8080/course/updateCourse/${courseId}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
